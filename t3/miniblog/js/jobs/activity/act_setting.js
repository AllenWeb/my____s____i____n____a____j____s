 ...
 * @modify xiongping1@staff.sina.com.cn   2010-6-29
 */
//import API
$import("diy/dom.js");
...
$registJob('actSetting', function(){
	...
	if(scope.$pid && scope.$pid != ""){
		$E('activity_pre_img').setAttribute('src', App.imgURL(scope.$pid,'thumbnail'));
		$E('activity_pre_panel').style.display = '';
		items['imgid'].value = scope.$pid;
	};
    var handler = {
        pageInit: function(){
			...
			//省份初始化
			if(scope.$extprov && scope.$extprov != ""){
				for(var i=0,selectLength = items['tprov'].getElementsByTagName("option").length;i<selectLength;i++){
					if(items['tprov'].getElementsByTagName("option")[i].value == scope.$extprov){
						items['tprov'].getElementsByTagName("option")[i].selected = true;
						var _temp = new App.ProvinceAndCity(items['tprov'],items['tcity'],scope.$extprov)
						_temp.init();
					}
				}
			};
			//城市初始化
			if(scope.$extcity){
				for(var i=0,selectLength = items['tcity'].getElementsByTagName("option").length;i<selectLength;i++){
					if(items['tcity'].getElementsByTagName("option")[i].value == scope.$extcity){
						items['tcity'].getElementsByTagName("option")[i].selected = true;
					}
				}
			};
        },
...
//						}
        ...
        postValidate: function(){
            var funs = ['checkClassSelected', 'compareDate', 'checkTopArea', 'checkActivityArea', 'checkPrice', 'checkActivityVenue'];
