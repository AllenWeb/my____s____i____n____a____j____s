...
function SUDA(config,ext1,ext2){
	...
	var _S_T1=100,_S_T2=500;var _S_TEntry="_s_tentry";
	var handler={
		...
		_S_sTEntry:function(){
			var e="-";if(""==SA._S_gUCk(_S_TEntry)){if(""!=_S_PREF_){e=SA._S_gHost(_S_PREF_);}SA._S_sUCk(_S_TEntry,e,'','t.sina.com.cn');}
			...
			if(_S_PURL_.match(vlogin)){
						...
						if(''!=sharehost){SA._S_sUCk(_S_TEntry,sharehost,'','t.sina.com.cn');}
						SA._S_sUCk('appkey',appkey,'','t.sina.com.cn');
			}
		},
		_S_gSPR:function(){var uoc=handler._S_gUOR();var upa=uoc.split(",");if(upa.length>=3){return upa[2];}else{return "";}},
		_S_upExt1:function(){
			var reg_arr=new Array(
					/t.sina.com.cn\/reg.php/,
					/t.sina.com.cn\/reg\/reg_succ.php/,
					/t.sina.com.cn\/reg\/reg_active.php/,
					/t.sina.com.cn\/person\/full_info.php\?.*type=3.*/,
					/t.sina.com.cn\/person\/guide_interest.php\?.*type=3.*/,
					/t.sina.com.cn\/person\/guide_invite.php\?.*type=3.*/,
					/t.sina.com.cn\/person\/full_info.php\?.*type=2.*/,
					/t.sina.com.cn\/person\/guide_interest.php\?.*type=2.*/,
					/t.sina.com.cn\/person\/guide_invite.php\?.*type=2.*/,
					/t.sina.com.cn\/reg_sinamail.php/,
					/t.sina.com.cn\/person\/full_info.php\?.*type=1.*/,
					/t.sina.com.cn\/person\/guide_interest.php\?.*type=1.*/,
					/t.sina.com.cn\/person\/guide_invite.php\?.*type=1.*/,
					/v.t.sina.com.cn\/widget\/full_info.php\?.*type=4.*/,
					/v.t.sina.com.cn\/share\/share.php\?.*type=4.*/
				);
	...
	if(_S_SMC==0){if('MSIE'==_S_NAN_){SSL.Global.doc.attachEvent("onclick",handler._S_MEvent);SSL.Global.doc.attachEvent("onmousemove",handler._S_MEvent);SSL.Global.doc.attachEvent("onscroll",handler._S_MEvent)}else{SSL.Global.doc.addEventListener("click",handler._S_MEvent,false);SSL.Global.doc.addEventListener("mousemove",handler._S_MEvent,false);SSL.Global.doc.addEventListener("scroll",handler._S_MEvent,false)}};handler._S_sUOR();handler._S_sTEntry();
	return{
		_S_pSt:function(pid,ext1,ext2){try{if((SA._S_isFreshMeta())||(SA._S_isIFrameSelf(_S_IFH,_S_IFW))){return}if(_S_ET>0){return}++_S_ET;setTimeout(function(){handler._S_gsSID()},_S_T1);setTimeout(function(){handler._S_pBeacon(pid,((undefined==ext1)?handler._S_upExt1():ext1),ext2,0)},_S_T2)}catch(e){}},
