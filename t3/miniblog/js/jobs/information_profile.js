...
function iniForm(){
	...
	var _showbox = $E('information_show');
	...
	//var _sonicnotice = $E("sonicnotice");
	var _havShowForm = false;
	var _isLayer = $E('isLayer');
    //将全局的方法私有化
    ...
    var showForm = function(){
	    if (/boxM_1/g.test(_showbox.className)) {
			_showbox.className = _showbox.className.replace(' boxM_1', '');
			_isLayer.style.display='none';
			App.refreshCheckCode();
			if (GB_SUDA) {
				
				if(scope.$pageid === 'profile'){
					GB_SUDA._S_uaTrack('tblog_reg', 'vip_reg');
				}
				if(scope.$pageid === 'mblog'){
					GB_SUDA._S_uaTrack('tblog_reg','content_reg');
				}
				if(scope.$pageid === 'search'){
					GB_SUDA._S_uaTrack('tblog_reg','topic_reg');
				}
			}
		}
		_havShowForm = true;
    ...
    var checkFunction = {
        ...
        'MR0004': function(el){//判断是新浪邮箱邮件
            if (App.Checkinfo.check(['MR0000', 'MR0002'])) {
        ...
        'MR0005': function(el){//判断是否已经注册
            el.ajaxCheck = '1';
            if (App.Checkinfo.check(['MR0000', 'MR0002', 'MR0004'])) {
				var _parm = {
                    "username": el.value
                };
    ...
    App.Checkinfo = App.checkForm(App.checkFormUI3);
    //邮箱地址
    App.Checkinfo.add('MR0000', _username, _red_username, checkFunction['MR0000']);
...
//	}
    ...
    App.bindFormTips([{
        ...
        'el': _realname,
        'key': 'R01102',
        'errorPos': _red_realname
    }, {
        'el': _idcard,
        'key': 'R01101',
        'errorPos': _red_idcard
    }]);
    //全局的函数
    App.information = {};
    //提交函数
                    ...
                    if (json.code == 'A00006') {
                        ...
                        if (json.code == 'M00004') {
                            ...
                            if (json.code == "MR0050") {
                                ...
                                if (json.code == "R01409") {
                                    _red_door.innerHTML = '<div class="iswhat iserro" style=""><strong><span>' + $SYSMSG[json['code']] + '</span></strong></div>';
    ...
    _addEvent(_submit, App.information['submit'], 'click');
    _addEvent(_username,showForm,'focus');
    //设定页面加载默认焦点
    //_username.focus();
    //_addEvent(_sonicnotice,function(){Core.Events.fireEvent(_username,'focus')},'click');
    /*
     * 用户防止乱注册
     */
...
$registJob('information3', iniForm);
