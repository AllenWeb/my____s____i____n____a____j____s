/**
 * @update by zhangjinlong		jinlong1@staff.sina.com.cn
 */
$import("sina/sina.js");
$import("sina/app.js");
$import("sina/jobs.js");
...
(function(){
	...
	//var maxlen = 60;
    $registJob('relation_search_tab', function(){	
        var list = [];		
        _each($E('guess_tab').getElementsByTagName('li'), function(v, i){
            list.push({
                'tab': v
            })
        })
    ...
    function getHTML(type){
		App.doRequest({ 'module' : type + 1 }, '/invite/aj_module.php', function(data){
				if( data ){
					$E('guess_module').innerHTML =  data.html;
					setTimeout(function(){
 ...
 //---------------增加按昵称搜索标签
	...
	var typeProxy = ['nickname','hobby','school','work','msn','mail','location'];
    var TabReady = {
		nickname:function(){	//---------add nickname search
			initInput(['nickname_input']);
			_addEvent($E('nickname_btn'),function(){
				locationToSearch({
					s_tag : getValue('nickname_input')     
				},true)
			},'click');
			
			(function(){
			var spec = {};
			spec['input'] = $E('nickname_input');
            spec['ok'] = function(value, text){
                spec['input'].value = text;
                Core.Events.fireEvent($E('nickname_btn'), 'click');
            };
			...
			App.enterSubmit({
                parent: $E('nickname_input').parentNode,
                action: function(){
                    Core.Events.fireEvent($E('nickname_btn'),'click');
                }
            });
