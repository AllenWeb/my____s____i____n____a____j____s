...
$registJob('hotlist',function(){
	try{
		var date = [];
		try{
			date = scope.$selectData.split('-');
		}catch(ex){}
		...
		new domkey.Date(
			box,
			function(y,m,d){
				if(scope.$pageid == 'square_hottopic'){
					window.location.href = '/pub/hottopic&date=' + y + '-' + (parseInt(m)+1) + '-' + d;
					return false;
				}
				if(scope.$pageid == 'hotlist'){
					window.location.href = '/hottopic/hottopic.php?date=' + y + '-' + (parseInt(m)+1) + '-' + d;
					return false;
				}
			},
