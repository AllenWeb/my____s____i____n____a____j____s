...
//		$Debug("开始:: 设定参数", {color: "#9f0"});
		...
		$Debug("对象:: " + this._node.id || "无ID名称", {
			color: "#ff99cc"
		});

		$Debug("属性[" + getType(this._property) + "]:: " + this._property, {
			color: "#ff99cc"
		});

		$Debug("起始值[" + getType(this._startvalue) + "]:: " + this._startvalue, {
			color: "#ff99cc"
		});

		$Debug("目标值[" + getType(this._endingvalue) + "]:: " + this._endingvalue, {
			color: "#ff99cc"
		});

		$Debug("单位[" + getType(this._suffixvalue) + "]:: " + this._suffixvalue, {
			color: "#ff99cc"
		});
		//*/
	...
	ancestor.prototype.play = function () {
		...
		for(i = 0; i < _len; i ++ ) {
			...
			//$Debug(this._node + "|" + this._property[i] + "|" + (ani + this._suffixvalue[i]));
			//$Debug(nTime + "|" + this._startvalue[i] + "|" + (this._endingvalue[i] + this._startvalue[i]) + "|" + this._seconds + "|" + ani);
		}
		this._fps ++;
		this._func.tween();
		if(nTime == this._seconds) {
	...
	ancestor.prototype.stop = function () {
		...
		this._end = true;
		this._func.end();
//		$Debug("FPS:: " + parseInt(this._fps / this._seconds));
//		$Debug("停止:: 清理定时器", {color: "red"});
	};
	/**
	 * Tween类的复位方法
	 */
	ancestor.prototype.reset = function () {
//		$Debug("清理:: 清理并初始化数据", {color: "yellow"});
