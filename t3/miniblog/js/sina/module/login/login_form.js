...
Module.Login.Form = {
	...
	loadCheckImage : function() {
		...
		if(this.checkImg) {
			...
			if (typeof scope.loginVcodeBinded == "undefined") {
				Core.Events.addEvent("reloadCode", this.reloadCheckImage.bind2(this),"click");
	...
	check : function() {
		...
		var fg = true;
		var loginName = $E("login_name").value;
		var loginPass = $E("login_pass").value;
	...
	logStart : function(handle) {
		...
		if(this.check()){
			var info = {};
			info.loginname = $E("login_name").value;
			info.loginpwd = $E("login_pass").value;
			if(this.checkImg){
	...
	parse : function(_data) {
		...
		if(typeof _data == "string" || typeof _data.code != "undefined" && _data.code == "10011"){
			...
			if($E("login_save") && $E("login_save").checked){
				Utils.Cookie.setCookie("remberloginname", escape($E("login_name").value), 2400, "", "sina.com.cn");
			}
			else{
