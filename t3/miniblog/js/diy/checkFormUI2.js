...
(function(proxy){
	var common = [
		{'tagName':'DIV','attributes':{'class':'new_plus','id':'box'},'childList':[
		{'tagName':'DIV', 'attributes':{'class':'mentip'},'childList':[
			{'tagName':'DIV','attributes':{'class':'tpbg'}},
			{'tagName':'DIV','attributes':{'class':'maincent','id':'content'}},
			{'tagName':'DIV','attributes':{'class':'btbg'}}
		]}]}];
    ...
    proxy.checkFormUI2 = function(key, noError, affect, errorPos){
...
//                    }
//                    else {
                        errorPos.getElementsByTagName('SPAN')[0].innerHTML = $SYSMSG[key];
                    //}
        ...
        for (var i = 0, len = els.length; i < len; i += 1) {
            ...
            (function(k){
                Core.Events.addEvent(els[k]['el'], function(){
                    ...
                    //					}
                    //                    els[k]['el'].parentNode.className="inputhover"
					var posEl = els[k]['el'].parentNode.parentNode;
					...
					if (scope.$pageid !== 'full_info1') {
						...
						if (!els[k]['el'].value.length && els[k]['key'] && $SYSMSG[els[k]['key']]) {
							...
							var posLeft = parseInt(posEl.getAttribute('positionLeft'))||3;//var posLeft = 3;
							if (posLeft) {
