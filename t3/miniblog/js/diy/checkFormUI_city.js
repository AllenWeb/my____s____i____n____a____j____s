...
//	];
	var common = [{'tagName':'TABLE', 'attributes':{'class':'cudTs','id':'box','style':'width:249px'},'childList':[
					...
					if (!els[k]['el'].value.length && els[k]['key'] && $SYSMSG[els[k]['key']]) {
						...
						if($IE){
							if($IE6){
								...
								tips.box.style.top = (pos[1] -2) + 'px';
							}
						}else{
							tips.box.style.top = (pos[1] + 0) + 'px';
						}
						if(els[k]['positionLeft']){
							tips.box.style.left = pos[0] + els[k]['el'].offsetWidth + els[k]['positionLeft'] + 'px';
						}else{
							if($IE){
								if($IE6){
									tips.box.style.left = pos[0] + els[k]['el'].offsetWidth +10+ 'px';
								}else{
									tips.box.style.left = pos[0] + els[k]['el'].offsetWidth +8+ 'px';
								}
							}else{
								tips.box.style.left = pos[0] + els[k]['el'].offsetWidth +10+ 'px';
							}
