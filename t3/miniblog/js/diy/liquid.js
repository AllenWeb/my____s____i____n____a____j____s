/**
 * @fileoverview 图像液化渐变效果
 * @see source code(MIT License) :    http://devpro.it/code/200.html
 * @see demo1                    :    http://www.3site.eu/examples/liquid/
 * @see demo2                    :    http://www.3site.eu/examples/liquid/vaio.html
 * for example                   :
 * App.liquid({
        speed: 40,
        scale: 2000,
        src: "xxx/xx_img.png",
        target: document.getElementById("xxx_img_container"),
        direction: "bottom"
    });
 * */
$import("sina/sina.js");
...
(function(proxy){
    ...
    function onload(config){
        ...
        var wrap = config.target.appendChild(config.document.createElement("div")), div = wrap.cloneNode(true), width = this.width, height = this.height, img = this, $wrap = wrap.style, $div = div.style, $img = img.style, scale = config.scale || 2000, speed = config.speed || 10, left, top, $;
        config.wrap = wrap;//用于后续处理
        margin($wrap, $div, $img);
        fontSize($wrap, $div);
        position($div, $img);
    ...
    function $onload(config){
        ...
        var div = config.target.getElementsByTagName("div"), wrap = div[0], div = div[1], img = config.document.createElement("img"), $wrap = wrap.style, $div = div.style, $img = img.style, width = parseInt($wrap.width, 10), height = parseInt($wrap.height, 10), scale = config.scale || 2000, speed = config.speed || 10, left, top, $;
        position($img);
        margin($img);
        $img.zIndex = 1;
